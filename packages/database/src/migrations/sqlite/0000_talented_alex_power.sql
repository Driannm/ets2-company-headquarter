...
CREATE UNIQUE INDEX `cabins_game_ref_id_unique` ON `cabins` (`game_ref_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `chassis_game_ref_id_unique` ON `chassis` (`game_ref_id`);--> statement-breakpoint
...
CREATE TABLE `drivers` (
	...
	`heavy_skill` integer DEFAULT 0,
	`fragile_skill` integer DEFAULT 0,
	`urgent_skill` integer DEFAULT 0,
	`mechanical_skill` integer DEFAULT 0,
	`is_player` integer DEFAULT false,
	`rating` real,
	`salary_per_km` real,
	`sync_status` text DEFAULT 'pending',
	`updated_at` integer NOT NULL,
...
--> statement-breakpoint
CREATE UNIQUE INDEX `engines_game_ref_id_unique` ON `engines` (`game_ref_id`);--> statement-breakpoint
CREATE TABLE `garages` (
	`id` text PRIMARY KEY NOT NULL,
	`game_ref_id` text,
	`city` text NOT NULL,
	`country` text DEFAULT 'Europe' NOT NULL,
...
CREATE TABLE `transmissions` (
	...
	`data_path` text,
	`gears_count` integer,
	`has_retarder` integer DEFAULT false,
	`updated_at` integer NOT NULL
);
--> statement-breakpoint
CREATE UNIQUE INDEX `transmissions_game_ref_id_unique` ON `transmissions` (`game_ref_id`);--> statement-breakpoint
CREATE TABLE `trucks` (
	`id` text PRIMARY KEY NOT NULL,
	`game_ref_id` text,
	`brand` text NOT NULL,
	...
	`fuel_ratio` real DEFAULT 0,
	`fuel_capacity_liters` real DEFAULT 0,
	...
	FOREIGN KEY (`chassis_id`) REFERENCES `chassis`(`id`) ON UPDATE no action ON DELETE set null,
	FOREIGN KEY (`engine_id`) REFERENCES `engines`(`id`) ON UPDATE no action ON DELETE set null,
